...
class BlogPostGenerator:
    ...
    def fetch_unsummarized_articles(self):
        ...
        c = conn.cursor()
        c.execute("SELECT id, title, raw_text, topic, sub_topic, date FROM articles WHERE published = 0")
        articles = c.fetchall()
        conn.close()
        return articles
    ...
    def format_blog_post(self, title, raw_text, seo_title, meta_desc, keywords, date):
        ...
        blog_post += """
            ...
            <h2>Conclusion</h2>
            <p>The situation around {title} continues to evolve. Stay updated with the latest news and analysis on {keywords}.</p>
    ...
    def generate_blog_posts(self):
        ...
        for article in articles:
            ...
            blog_post = self.format_blog_post(title, raw_text, seo_title, meta_desc, keywords, date)
            
            # Save the formatted blog post to the database
            c.execute("UPDATE articles SET full_post = ?, seo_title = ?, meta_description = ?, seo_keywords = ?, published = 1 WHERE id = ?", 
